                            ...
                            <div *ngIf="mpesaSubMethod === 'stk'" class="option-view animate-fade-in">
                                ...
                                <form [formGroup]="stkForm" (keydown.enter)="processStkPush()">
                                    <div class="phone-input-wrapper">
                                        <input type="tel" formControlName="phoneNumber" placeholder="+254722996540"
                                               class="phone-input" [disabled]="isProcessingStk" (keydown.enter)="processStkPush()" />
        ...
        this.quoteService.validatePaybillTransaction(mpesaCode)
            .subscribe({
                next: (res) => {
                    ...
                    if(res.debitnoteno){
                        if(res.debitnoteno.toUpperCase()!==this.data.reference.toUpperCase()){
                            this.showToast(
                                `Payment reference ${res.debitnoteno} entered on Mpesa does not match expected reference: ${this.data.reference}`
                            );
                            this.isVerifyingPaybill = false;
                            return;
                        }
                    ...
                    const paymentData = {
                        "proposalId":this.data.shippingId,
                    ...
                    this.quoteService.savePaybillPayment(paymentData).subscribe({
                        next: () => {
                            this.showToast(
                                `Dear customer, your payment of Ksh ${amount} for Marine Transaction reference ${this.data.reference} has been successfully received.`
                            );
                            this.isVerifyingPaybill = false;
                            setTimeout(() => {
                                this.dialogRef.close({
