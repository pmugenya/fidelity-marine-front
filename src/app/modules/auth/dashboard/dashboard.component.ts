...
@Component({
  ...
  styles: [`
    :host { --pantone-306c: #00787C; --pantone-2758c: #21275c; --white-color: #ffffff; --light-gray: #f8f9fa; --medium-gray: #e9ecef; --dark-gray: #495057; }
...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  recentActivities: Activity[] = [ { id: 'A001', title: 'Payment Successful', description: 'Travel Insurance for Europe', timestamp: new Date(Date.now() - 3600000), icon: 'payment', iconColor: '#00787C', relatedId: 'P003' }, { id: 'A002', title: 'Certificate Downloaded', description: 'Marine Cargo Policy MAR-2025-002', timestamp: new Date(Date.now() - 14400000), icon: 'download', iconColor: '#00787C', relatedId: 'P002' }, { id: 'A003', title: 'Profile Updated', description: 'Contact information updated', timestamp: new Date(Date.now() - 86400000), icon: 'person', iconColor: '#21275c' }];
  ...
  loadDashboardData(): void {
      ...
      this.userService.getClientQuotes(this.page, 4).subscribe({ // Load more quotes to ensure we get all
          next: (res) => {
             ...
             this.totalRecords = res.totalFilteredRecords;
    ...
    if (quote.prodName === 'Travel Insurance') {
        ...
        this.openShipmentDetailsPaymentModal(quote.quoteId, quote.status, quote.netprem, quote.phoneNo, quote.refno,quote.shippingId);
    }
}

  showOption(status: any):boolean{
      return status!=='PAID';
  }


    private openShipmentDetailsPaymentModal(shippingId: number, status: string, prem: number, phone: string, refno: string,shipId:number): void {
  ...
  dialogRef.afterClosed().pipe(takeUntil(this.destroy$)).subscribe(result => {
   ...
   this.loadDashboardData();
      ...
      this.openPaymentModal(prem,phone,refno,shipId);
  }
}
    ...
    private openPaymentModal(premium: any, phoneNo: any, ref: any, shippingId:any): void {
...
ngOnDestroy(): void {
  ...
  isQuoteExpiringSoon(quote: PendingQuote): boolean {
    return quote.daysToExpiry > 0 && quote.daysToExpiry <= 3; // Expiring within 3 days
  }
